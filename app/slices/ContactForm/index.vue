...
function composeMessage() {
  form.value.message = `Ciao Radici pugliesi,
Mi chiamo ${form.value.name} ${
    form.value.surname
  } e sto organizzando un evento per il prossimo ${formatDate(
    new Date(form.value.date),
    "DD MMMM YYYY"
  )}. 
Mi piacerebbe ricevere un preventivo per l'acquisto di ${form.value.quantity} "${
    form.value.articles
...
<template>
  ...
  >
    <div id="contact_form" class="max-w-4xl mx-auto">
      ...
      <form name="contact-form" method="post" data-netlify="true" @submit.prevent="send">
        <input type="hidden" name="form-name" value="contact-form" />
        <div v-if="messageSend" class="flex items-center gap-8">
          <Icon name="i-system-uicons:message" size="96" class="text-green-400" />
          <div class="">
        ...
        <div v-if="messageSend === false" class="flex items-center gap-8">
          <Icon name="i-system-uicons:message" size="96" class="text-red-400" />
          <div class="">
        ...
        <fieldset :disabled="messageSend" :class="{ 'opacity-20': messageSend }">
          ...
          <div class="relative">
            ...
            >
              <button class="sparkle" @click.prevent="composeMessage">
                <Icon name="i-system-uicons:translate" size="24" />
              </button>
            </div>
